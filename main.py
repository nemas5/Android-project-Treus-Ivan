...
class AnButt(ButtonBehavior, Image):
...
class ConvApp(App):
	...
	def build(self):
		...
		self.bx_l = BoxLayout(size_hint = (1, .15), pos_hint = {'top':.75, 'center_x':-.25})
	...
	def anim_l(self, btn):
		...
		if self.k % 2 == 1:
			...
			an.start(self.r_btn)
			an.start(self.bx_r)
			an = Animation(pos_hint = {'center_x':.49}, duration = .2)
			an.start(btn)
			an.start(self.bx_l)
			self.k += 1
		else:
